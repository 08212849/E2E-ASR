...
def cal_loss(pred, gold, smoothing=0.0):
    """
    Calculate the cross-entropy loss, applying label smoothing if specified.

    Parameters:
    - pred (torch.Tensor): (batch_size, classes),
    - gold (torch.Tensor): Ground truth labels, with shape (batch_size,).
    ...
    if smoothing > 0.0:
        ...
        one_hot = torch.zeros_like(pred).scatter(1, gold_for_scatter.view(-1, 1).long(), 1)
